...
> 每個 Phase 的 Verification 必須通過才能進下一個。**時間不夠就從 Phase 順序末端往前砍**：Phase 6 (Bonus) → Phase 4 (Multi-turn NL) → Phase 5 (PUT UI + Error Polish)。Phase 1 / 2 / 7 是 Must-have。
...
3. Phase 5 子項 (D#26 PUT UI) — Delete + recreate 仍可用。
4. Phase 3 子項 (D#28 Per-rule run) — 保留 run-all，rule_ids 留為 API surface 但 frontend 不暴露。
...
4. 三大評分項對應的「最強展示點」：
   ...
   - **Product Thinking**：三色 status（D#18）、honest design（D#22）、edit + diff view（D#26）、每 code 都有 possible_causes（D#31）。
...
Future Enhancements（皆寫入 `docs/architecture.md`）：D#19 draft 持久化、D#22 UNIQUE 約束、Celery / 多 process、PII masking、multi-user auth、mobile layout、cache GC cron job、cross-rule expectations。
